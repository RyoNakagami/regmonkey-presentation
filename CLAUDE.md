...
- **`style/revealjs.scss` + `style/scss/_*.scss`** — the theme actually loaded by decks (`posts/_metadata.yml` sets `theme: ../../style/revealjs.scss`, which `@import`s the partials in `style/scss/`). Every visual element on a slide (pentagon-box, square-box, info-box / success-box / warning-box / caution-box / danger-box, hop-step-jump, summary blocks, component-cards, mini-section, h2-submessage, border-bottom-headers, plus utility classes like `.font-09`–`.font-20`, `.lh-07`–`.lh-20`, `.padding-left-05`, `.width-110`, `.position-left-XX`, `.regmonkey-bold`, `.squaredmark`) is a class defined here. `style/revealjs.css` is a legacy flat copy that is NOT loaded by slides — when adding or changing a class, edit the scss partial (keep the css copy in sync if you touch overlapping rules). **Do not invent class names or write raw HTML/inline styles** — they will not match the rest of the site.
...
3. A class name that doesn't exist in `style/scss/` partials (silently no-op — search the scss before inventing names). Note: scss partial edits may not bust Quarto's sass compile cache immediately; if a new class doesn't show up, touch `style/revealjs.scss` or restart the preview.
