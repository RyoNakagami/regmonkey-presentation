...
::::{ .width-105 .position-left-10 .font-08}
...
[人間への指示と同じ原則にAI固有の留意点を組み合わせる]{.h2-submessage}
...
- プロンプトエンジニアリング = [AIから望ましい応答を引き出すための指示文を設計する実践]{.regmonkey-bold}
- 出力の質は [プロンプトの具体性・構造・前提情報の与え方]{.regmonkey-bold} に強く依存する
...
[AI Fluency 4D Framework における位置づけ[^footer-prompt-4d]]{.mini-section}

{{< reveal_vspace 0.25em >}}

:::{.font-09 .padding-left-05 .lh-14}

- プロンプティングは [Description（AIとの効果的な意思疎通）]{.regmonkey-bold} に該当する技能
...
[^footer-prompt-4d]: 4D Framework の詳細は Appendix の「AI Fluencyの4D Framework」スライドを参照

## 人間への依頼との共通点と相違点を理解する

[文脈・例示・制約は共通．役割設定・思考時間はAI固有の考慮]{.h2-submessage}
...
- [背景・目的の説明]{.regmonkey-bold}：何のために何が欲しいかを冒頭で示す
- [望ましい出力の例示]{.regmonkey-bold}：完成イメージのサンプルを添える
- [要件・制約の明文化]{.regmonkey-bold}：フォーマット・分量・口調を指定する
- [複雑な依頼の分解]{.regmonkey-bold}：手順や中間成果物を順序づけて渡す
...
- [役割・トーンの定義]{.regmonkey-bold}：「あなたは〜の専門家」と立場を与える
- [推論時間の確保]{.regmonkey-bold}：結論前に思考過程を書かせる(`Think step-by-step`)
- [反復改善が前提]{.regmonkey-bold}：初稿を見て指示を調整する協働プロセス
- [AI自身に改善依頼]{.regmonkey-bold}：プロンプトの不足点をAIに指摘させる
...
[文脈・例示・制約・分解・思考・役割を組み合わせて使う]{.h2-submessage}

{{< reveal_vspace 0.5em >}}

::::{.custom-table style="width:100%; height:75%; font-size: 0.85em !important;"}
:::{.yaml2table .yaml2table-custom-top #yaml-prompting-techniques data-col-widths="[23, 32, 45]"}
...
[1 MTok = 100万トークン；Input・Cache Write・Cache Hit・Output それぞれの消費量に課金される]{.h2-submessage}

{{< reveal_vspace 1em >}}

[モデル別コストテーブル(単位はすべてMTok)]{.mini-section}

::::{.custom-table style="width:100%; height:50%; font-size: 1em !important;"}
:::{.yaml2table .yaml2table-custom-top #yaml-api-pricing data-col-widths="[30, 12, 13, 13, 13, 19]"}
...
[Claude Sonnet 4.6：入力10万tokens・出力2,000tokensの場合；キャッシュ比率90%で合計コストが\$0.330 → \$0.087]{.h2-submessage}
...
キャッシュなし（全量を新規入力）
:::
:::{.squaredmark style="font-size: 0.90em"}
- Input 100,000 tokens
  - \$3 per MTok  × 100,000 = [\$0.300]{.regmonkey-bold}
- Output 2,000 tokens
  - \$15 per MTok × 2,000 = [\$0.030]{.regmonkey-bold}

{{< reveal_vspace 0.3em >}}

[合計：\$0.330 / リクエスト]{.regmonkey-bold}
:::
::::
:::::
::::: {.column width="50%"}
...
- Base Input 10,000 tokens（新規）
  - \$3 per MTok × 10,000 = [\$0.030]{.regmonkey-bold}
- Cache Hit 90,000 tokens（ヒット）
  - \$0.30 per MTok × 90,000 = [\$0.027]{.regmonkey-bold}
- Output 2,000 tokens
  - \$15 per MTok × 2,000 = [\$0.030]{.regmonkey-bold}

{{< reveal_vspace 0.3em >}}

[合計：\$0.087 / リクエスト（約74%削減）]{.regmonkey-bold}
:::
::::
:::::
::::::
...
::: {.info-contents .font-10 .padding-left-10}
