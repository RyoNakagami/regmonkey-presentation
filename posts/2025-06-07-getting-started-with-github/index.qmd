...
::: {style="font-size:2em; padding-left:0.5em;" }
...
- FileやDirectoryのPATHを指定すれば，current branchではなく，指定したPATHに絞ったlogを同じ慶して出力できます
...
`--pretty=format` オプション解説

:::

:::{style="font-size:0.9em; padding-left:0.5em;" }

| フィールド             | 説明                 |
| :-----------------: | ------------------ |
| `%cs`             | コミット日時をYYYY-MM-DD 形式で表示 |
| `%ci`             | コミット日時をISO 8601的形式で表示 |
| `%h`              | 短縮コミットID           |
| `%an`             | Author名            |
| `%s`              | コミットメッセージ          |
: {tbl-colwidths="[30,70]"}
