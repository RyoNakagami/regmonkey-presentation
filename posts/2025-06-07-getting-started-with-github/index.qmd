...
[`git log` コマンドの基本動作]{.info-box-title}

::: {.info-contents .pl-5}

- **Syntax**: 
  - `git log [options] -- <file>`
- 誰がいつリポジトリにcommitして，どのような差分が発生したのかを確認できる(=`git rebase`や`git difftool`を使いこなすために必要)
...
:::: {.columns}
::: {.column width="47.5%" .font-08}

[`git log` の挙動例]{.mini-section}
...
:::
::: {.column width="52.5%" .font-08 .pl-5}

[`git log` options]{.mini-section}

| **オプション**                         | **説明**                                              |
| --------------------------------- | --------------------------------------------------- |
| `--pretty=format:"%cs %h %an %s"` | `%cs`: コミット日時（YYYY-MM-DD）<br>`%h`: 短縮コミットID<br>`%an`: Author名<br>`%s`: コミットメッセージ |
| `-p`                              | 各コミットにおける差分（パッチ）を表示                              | 
| `--after=<日付>`  | `--after="2025-01-01"` で2025-01-01以降にフィルタリング（境界含む） |
| `--before=<日付>`  | `--before="2025-01-01"` で2025-01-01以前にフィルタリング （境界含む）|
...
[.gitignoreファイル]{.info-box-title}

::: {.info-contents .pl-5}

- 不必要なcommitを避けることを目的にGitによる追跡対象としないファイルを指定するための設定ファイル
...
- すでに一度 git add を経験したファイルは、インデックスに追加され、Git 追跡対象に含まれてしまうので、後から.girignoreを変更しても意図した通りの動きをしないときがある

:::

{{< reveal_vspace 1em >}}

[すでに追加されたファイルの追跡を解除したい場合]{.mini-section}
...
- `.gitignore` に書かれたファイルを「今後追跡しない」状態に変更
- ファイルはローカルには残る（削除されない）

:::



## Windows + PowershellでのGit環境構築
[環境構築]{.topleftbox}
...
[1]{.step-number} [**公式サイト**](https://git-scm.com/download/win)からインストーラーをダウンロード
...
[2]{.step-number} インストールウィザードに従ってセットアップを進める

:::

::: {.callout-note .pl-15 .font-12}
### [Git Bash onlyを選択]{.pl-2}

「Git Bash only」を選択しておくと環境がシンプルになります
...
[3]{.step-number} インストール完了後，PowerShell を開きPATHの確認
...
- バージョン情報（例：git version 2.46.0.windows.1）が表示されれば成功
...
[4]{.step-number} PowerShellでのGitレポジトリ作成テスト
...
PS> git init
PS> cd ..
PS> Remove-Item ./test -Recurse -Force
```
:::

- 上記がエラーなく実行できれば、Git の動作確認は完了
...
- `git.exe`の存在場所を確認する
- 通常は `D:\Program Files\Git\bin\git.exe`

:::{.font-13}

```powershell
Test-Path "D:\Program Files\Git\bin\git.exe"
```
...
- バージョン情報が出力されれば，次のステップ
- 出力されない場合はGitの再インストール
:::
...
- `Test-Path $PROFILE` で表示されたprofileファイルに以下のラインを追加
