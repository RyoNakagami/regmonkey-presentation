...
::: {.regmonkey_index style="width:95%" .padding-left-10}
...
::: {.info-contents .font-08 .padding-left-05 .lh-12 style="margin-right:2em"}
...
:::{.yaml2table .yaml2table-custom-top #yaml-app-protocols data-col-widths="[18, 56, 26]"}
...
[^addressclass]: IPアドレスクラスとは，IPアドレスをネットワーク規模に応じてクラスA (`/8`: 大規模ネットワーク)，クラスB (`/26`: 中規模)，クラスC (`/24`: 小規模)に分類する方式のこと
...
:::
::: {.column width="35%" .padding-left-10}
...
[数字のIPアドレスとドメイン名を対応させるサービス]{.h2-submessage}

:::: {.columns}
::: {.column width="70%"}

![](./dns-name-resolution.svg){width=100%}
...
- ブリッジ・L2スイッチ・無線LANアクセスポイントは[MACアドレステーブルを学習]{.regmonkey-bold}し，宛先のあるポートにのみ転送する
...
::::{.custom-table style="width:100%; font-size: 0.68em !important;"}
:::{.yaml2table .yaml2table-custom-top #yaml-datalink-devices data-col-widths="[16, 26, 30, 28]"}
...
record1:
  機器:
    - リピータハブ<br>（物理層）
  役割:
    - 受信した電気信号を増幅・整形し，受信ポート以外の全ポートへ転送
  転送の仕組み:
    - MACアドレスを見ない
    - 宛先に関係なく全ポートに同じ信号を流す
  備考:
    - 全ポートが同一の衝突ドメイン・半二重通信
    - 現在はほぼL2スイッチに置き換えられている

record2:
  機器:
    - ブリッジ
  役割:
    - LANセグメント同士を接続し，フレームを中継
  転送の仕組み:
    - 送信元MACアドレスからポートとMACアドレスの対応を学習
    - 宛先MACアドレスが存在するセグメントにのみ転送
  備考:
    - 衝突ドメインを分割できる
    - ソフトウェア処理が中心で，機能はL2スイッチに引き継がれた

record3:
  機器:
    - L2スイッチ<br>（スイッチングハブ）
  役割:
    - 複数ノードをポート単位で収容し，フレームを高速中継
  転送の仕組み:
    - MACアドレステーブルに基づき宛先ポートにのみ転送
    - 専用ハードウェア（ASIC）で処理する多ポート版ブリッジ
  備考:
    - ポートごとに衝突ドメインを分割・全二重通信が可能
    - VLAN対応機種もある
...
record4:
  機器:
    - 無線LAN<br>アクセスポイント
  役割:
    - 無線端末を有線LANに接続（無線と有線のフレーム変換）
  転送の仕組み:
    - MACアドレスに基づき無線・有線間を中継（ブリッジとして動作）
  備考:
    - 無線区間はCSMA/CAによる半二重通信
    - IEEE802.1Xの認証装置としても機能する
```
...
- MAC アドレステーブル・ARP テーブル・ルーティングテーブルを内部に持ち、それらを ASIC[^asic] によってハードウェア処理しているため，VLAN 間ルーティングを「高速に」行える
...
![](./l3-switch-inter-vlan.svg)


:::{.font-08}

- 受信した[Ethernetフレームを一旦取り除き]{.regmonkey-bold}，中のIPパケットの宛先IPアドレスから転送先VLANを決定する
- 送出時は宛先MACアドレスをARPテーブルで解決し，[新しいEthernetフレームを付け直して]{.regmonkey-bold}送り出す
...
[^asic]: Application Specific Integrated Circuit．決められた処理だけを非常に高速に実行する専用プロセッサ．
...
![](./lan-utilization-setup.svg){width="100%"}

:::{.font-07 .padding-left-20}
...
[LAN利用率: 約12.5%]{.mini-section}

:::{.font-09 .padding-left-10}

1. 1回の転送で流れるデータ量：ファイル1000 byte ＋ 制御情報30% → 1000 × 1.3 = 1300 byte

{{< reveal_vspace 0.5em >}}

2. ビットに換算：1300 byte × 8 = 10,400 bit

{{< reveal_vspace 0.5em >}}

3. 1ペアあたりのトラフィック：10,400 bit × 60回/s = 624,000 bps

{{< reveal_vspace 0.5em >}}

4. LAN全体のトラフィック：(A,B)と(C,D)の2ペアが同一LANを共有するので 624,000 × 2 = 1,248,000 bps

{{< reveal_vspace 0.5em >}}

5. 利用率：1,248,000 ÷ 10,000,000 = 0.1248 ≒ 12.5%
