...
::: {.column style="width:70%; padding-left:1em;"}
...
## Skill は「成果物・タスク・ドメイン」の3観点で切り分ける

[「何を作るか」「どう動くか」「どの領域か」のどれを軸にするかで Skill の粒度と再利用性が決まる]{.h2-submessage}
...
:::{.block-header}
{{< bi file-earmark-text-fill size=1.6em color=#428CE6 >}} 成果物単位
:::

{{< reveal_vspace 0.5em >}}

:::{.block .squaredmark style="font-size:0.85em; padding-right:0.5em;"}

- [**「何を作るか」**]{.regmonkey-bold}で Skill を切る
- 最終的に生まれるファイル・ドキュメントのフォーマットを軸にする
- 例：API仕様書，テスト計画書，運用手順書
...
:::{.block-header}
{{< bi gear-fill size=1.6em color=#428CE6 >}} タスク単位
:::
...
- [**「どういう作業をするか」**]{.regmonkey-bold}で Skill を切る
- 手順・判断分岐・典型的な失敗回避を資産化できる
- 例：パフォーマンステスト実行，DBマイグレーション，セキュリティスキャン
...
:::{.block-header}
{{< bi globe2 size=1.6em color=#428CE6 >}} ドメイン単位
:::
...
- [**「どの分野・領域か」**]{.regmonkey-bold}で Skill を切る
- 専門用語・業務慣習・固有ルール・規制要件を集約
- Claude の事前知識が古い・不足する領域を埋めるのに有効
...
## 複数観点が重なる実タスクは，層に分けた Skill を組み合わせる

[一つの巨大 Skill に詰め込むより，観点ごとに分けるほうが再利用性も保守性も上がる]{.h2-submessage}
...
- 例：「規制対象プロダクトのリリース前チェック」は [**ドメイン × タスク × 成果物の3層が同時に走る**]{.regmonkey-bold}
- 1つの Skill に詰め込むと [**他のプロダクトや別タスクで再利用できなくなる**]{.regmonkey-bold}
- 層ごとに分けておけば [**ドメインだけ差し替え・タスクだけ流用**]{.regmonkey-bold}が可能になる
...
[3層に分解した例：規制対象プロダクトのリリース前チェック]{.mini-section}

{{< reveal_vspace 0.5em >}}

::::{.custom-table style="width:100%; font-size: 0.9em !important;"}
:::{.yaml2table .yaml2table-custom-top #yaml-skill-design-layers data-col-widths="[20, 32, 48]"}
