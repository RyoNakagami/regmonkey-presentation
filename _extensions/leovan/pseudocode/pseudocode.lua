local function ensure_html_deps()
  ...
  quarto.doc.include_text("in-header", [[
    ...
    <style>.ps-comment { padding-left: 1em; }</style>
  ]])
  quarto.doc.include_text("after-body", [[
    <script type="text/javascript">
    function _pseudocodeRenderAll() {
      // Patch KaTeX to support commands not natively available
      if (typeof katex !== "undefined" && !katex._pseudocodePatched) {
        var _origRenderToString = katex.renderToString;
        katex.renderToString = function(tex, options) {
          options = options || {};
          options.macros = Object.assign(options.macros || {}, {
            "\\textsc": "\\textsf"
          });
          return _origRenderToString.call(katex, tex, options);
        };
        katex._pseudocodePatched = true;
      }

      document.querySelectorAll(".pseudocode-container").forEach(function(el) {
        if (el.querySelector(".ps-root")) return;
        var pseudocodeOptions = {
      ...
      document.querySelectorAll(".pseudocode-container").forEach(function(el) {
        var captionSpan = el.querySelector(".ps-root > .ps-algorithm > .ps-line > .ps-keyword")
        if (captionSpan !== null) {
          var captionPrefix = el.dataset.captionPrefix + " ";
          var captionNumber = "";
    ...
    if (document.readyState === "complete") {
      _pseudocodeRenderAll();
    } else {
      window.addEventListener("load", _pseudocodeRenderAll);
    }
    </script>
  ]])
end
