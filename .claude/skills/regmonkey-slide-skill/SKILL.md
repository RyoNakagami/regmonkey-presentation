...
- **`references/wording.md`** — 推奨用語変換（`定石` → `一般的`，誇張語の事実ベース化，口語 → 書き言葉，等）．本文を執筆中に語感に違和感を覚えたとき，または推敲フェーズで読み返したときに開く．SKILL.md の「ハウスライティングルール」を補完する語彙レベルのチートシート．
- **`templates/*.qmd`** — 即コピペ可能なテンプレート9点．コピーして編集すること．書き直さない．
- **`subagents/slide-svg-generator.md`（サブエージェント）** — フローチャート・関係図・概念図など SVG を新規作成／既存 SVG をリスタイルするときに開く．ハウススタイル（枠 `#0E3666` / 透明背景 / 黒文字 / viewBox 1920基準）の完全仕様・テンプレート・チェックリスト・アンチパターンを保持．本 SKILL.md には最小ルールのみ記載（`## SVG / フローチャート / 図` セクション参照）．
...
- **H2 タイトルは33文字以内が望ましい．** H2（`##`）はテイクアウェイ文だが，長すぎるとスライド最上部で折り返し，視認性が落ちる．33字を超えそうなら本体を短くまとめ，補足は直下の `[...]{.h2-submessage}` に逃がす．例：「MLflow を導入してモデル管理・実験ログ・成果物アーカイブを統合的に自動化する」（39字）→ H2 に「MLflow でモデル管理を自動化」（13字），h2-submessage に「実験ログ・成果物アーカイブも統合的に取り込む」．
...
脚注は **anchor をスライド本文に，定義は 同スライド の一番最後にまとめて置く．** スライドごとに定義を集約する
...
- `<!-- footer -->` HTML コメントを区切りに置く．後で全脚注定義を grep / 編集するときの目印になる．
- Quarto Reveal はこの定義を解釈して，anchor がある各スライドの下端に小さく脚注番号付きで描画する．anchor の隣に定義を書く必要はない．
- スライド途中（例：あるスライドの直下）に脚注定義を置くと，HTML 出力が崩れる場合がある．**必ず同スライドの末尾に集約する．**
...
スライドにフローチャート・関係図・概念図・タイムライン等を入れるときは **SVG で書く**．ラスター画像（PNG/JPG）と違ってテキスト検索でき，拡大しても粗くならず，本文と同じトーンで揃う．**詳細仕様・テンプレート・チェックリストは `subagents/slide-svg-generator.md`（サブエージェント）が保持しているのでそこを開く．** 本セクションには最小ルールのみ記載：

**最小ルール（守らないと統一感が崩れる）：**

- ブロックの枠線：`stroke="#0E3666"`
- ブロックの背景：`fill="none"`（透明）
- 全テキスト：`fill="#1A1A1A"`（黒）．`<style>` で一括指定し，個々の `<text>` に色 fill を書かない
- フォント：`Meiryo` を含む font-family を `<style>` で明示
- viewBox 幅は **1920 を基準**（スライド全幅 100% = 1920 design unit）．`viewBox="0 0 1920 H"` と書き，embed `width="100%"` でスライド全幅 1600px に充てる
...
**こういうときに `subagents/slide-svg-generator.md` を開く：** 新規 SVG をゼロから起こすとき / 既存 SVG をハウススタイルにリスタイルするとき / 矢印 marker やフォント指定の完全な雛形が必要なとき / 完了前のチェックリストが必要なとき．インラインの最小ルールだけでは不安なら開く．

## スライドの height overflow を防ぐ

キャンバスは **1600 × 900** 固定．H2（テイクアウェイ）と `h2-submessage`，フッター・ページ番号で約 250px が消費されるため，**本文の作業領域は約 650px** しか残らない．Reveal.js は overflow をスライド内スクロールにするため，ブラウザでパッと見た限りでは気づきにくく，PDF 出力（`reveal2pdf.sh`）では truncate されて情報が消える．完了宣言の前に必ず render して目視確認する．

### overflow を起こしやすいパターン

- **info-box（上段）+ 2列の `square-box-500`（下段）の縦積み．** 2要素合計で 600px 超になり，`h2-submessage` の存在で確実にあふれる．→ `square-box-500` を `-450`/`-400` に下げるか，上段の info-box を `h2-submessage` に統合する．
- **第一階層の箇条書きが 7 項目以上．** MMUF に立ち返ると大抵は不要な項目が混じっている．6 項目以下に削るか，2スライドに分割する．
...
5. **最後の手段．** `style="font-size: 0.85em"` をブロックに直接付与．これは読みやすさを犠牲にするので，他の選択肢を試した後に限る．

### 確認手順

執筆ワークフロー step 5（完了宣言の前のチェック）に以下を含める：

1. `quarto render posts/<slug>/index.qmd` を実行する．
2. ブラウザでスライドを開き，**全スライド**を順に送って overflow を目視する．スクロールバーが出ているスライドが overflow している．
3. PDF 出力する deck であれば `reveal2pdf.sh` で生成して PDF も確認する（PDF では truncate される）．
